...
async function bootstrap() {
  initTelemetry({ serviceName: 'teamspace-one-interview-service' });

  const pino = createLogger({ name: 'interview-service' });
  ...
  const port = config.get<number>('PORT', 3014);

  await app.listen(port);
  pino.info(`Interview service listening on port ${port}`);
}
