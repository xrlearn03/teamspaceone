...
async function bootstrap() {
  initTelemetry({ serviceName: 'teamspace-one-service-template' });

  const pino = createLogger({ name: 'service-template' });
  ...
  pino.info(`Service template listening on port ${port}`);
}
