...
async function bootstrap() {
  initTelemetry({ serviceName: 'teamspace-one-notification-service' });

  const pino = createLogger({ name: 'notification-service' });
  ...
  pino.info(`Notification service listening on port ${port}`);
}
