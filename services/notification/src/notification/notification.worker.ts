...
export class NotificationDeliveryWorker extends WorkerHost {
  ...
  async process(job: Job<{ deliveryId: string }>): Promise<void> {
    ...
    try {
      switch (delivery.channel) {
        case 'email':
          await this.sendEmailMessage(delivery.notification);
          break;
        case 'desktop':
  ...
  private async sendEmailMessage(notification: { id: string; title: string; body: string; userId: string; eventType: string; link?: string | null; organisationId?: string; actorId?: string | null }): Promise<void> {
    ...
    if (!to) {
      this.logger.warn(
        ...
        'No recipient email available; falling back to console log',
      );
      this.logger.log(
  ...
  private async resolveRecipientEmail(notification: { userId: string; organisationId?: string; actorId?: string | null }): Promise<string | undefined> {
    const authUrl = this.config.get<string>('AUTH_SERVICE_URL');
    if (!authUrl) {
      return this.config.get<string>('EMAIL_FALLBACK_TO');
    }
    ...
    return user?.email ?? this.config.get<string>('EMAIL_FALLBACK_TO');
  }
