...
async function bootstrap() {
  ...
  initTelemetry({ serviceName: 'teamspace-one-messaging-service' });

  const pino = createLogger({ name: 'messaging-service' });
  ...
  pino.info(`Messaging service listening on port ${port}`);
}
