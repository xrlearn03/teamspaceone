...
async function bootstrap() {
  ...
  initTelemetry({ serviceName: 'teamspace-one-organisation-service' });

  const pino = createLogger({ name: 'organisation-service' });
  ...
  pino.info(`Organisation service listening on port ${port}`);
}
