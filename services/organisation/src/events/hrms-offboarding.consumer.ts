...
const SUBJECT = 'teamspace-one.hrms.employee.terminated';
const DURABLE = 'organisation-hrms-offboarding';

/**
 * HRMS → organisation offboarding: when an employee is terminated in HRMS,
 * remove their organisation membership so collaboration access is revoked
 * without a manual admin step.
 */
@Injectable()
export class HrmsOffboardingConsumer implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(HrmsOffboardingConsumer.name);
  ...
  async onModuleInit() {
    ...
    this.consume().catch((err) => {
      this.logger.error(`HRMS offboarding consumer error: ${(err as Error).message}`);
    });
  }
    ...
    for await (const msg of this.subscription) {
      ...
      try {
        ...
        if (typeof userId !== 'string' || !userId) {
          this.logger.warn('employee.terminated event missing payload.userId');
          jsMsg.ack();
          continue;
        }

        await this.inbox.handle(data, (tx) =>
          this.organisation.deactivateMembershipFromEmployee(tx, data.organisationId, userId),
        );
        jsMsg.ack();
      } catch (err) {
        this.logger.error(`Failed to process employee.terminated: ${(err as Error).message}`);
