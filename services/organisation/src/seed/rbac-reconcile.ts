...
export interface RbacReconcileResult {
 ...
 * Runs automatically on organisation-service startup and is also invoked by
 * the standalone `db:seed:rbac` script.
 */
export async function reconcileRbac(prisma: PrismaClient): Promise<RbacReconcileResult> {
