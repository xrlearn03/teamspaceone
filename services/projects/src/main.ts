...
async function bootstrap() {
  initTelemetry({ serviceName: 'teamspace-one-projects-service' });

  const pino = createLogger({ name: 'projects-service' });
  ...
  pino.info(`Projects service listening on port ${port}`);
}
