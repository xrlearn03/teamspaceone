...
export function GuestMeetingScreen({
  initialToken,
  onExit,
}: {
  initialToken?: string;
  ...
  // them, take the normal member route instead of the guest flow.
  useEffect(() => {
    if (!token) return;
    let cancelled = false;
    (async () => {
      try {
        ...
        const data = await getGuestMeetingInfo(token);
