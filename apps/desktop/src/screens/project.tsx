      ...
      <div className="flex h-full flex-col">
        ...
        {tab === "board" ? (
          <div className="flex flex-1 gap-3 overflow-x-auto p-4">{statuses.map((status) => (
            <div key={status} className="flex w-64 shrink-0 flex-col rounded-lg border bg-surface-elevated/50">
              ...
              <div className="flex-1 space-y-2 p-2">{grouped[status]?.map((task) => <TaskCard key={task.id} task={task} onClick={() => setSelectedTask(task)} />)}{!grouped[status]?.length ? <div className="rounded-md border border-dashed py-6 text-center text-xs text-text-muted">No tasks</div> : null}</div>
...
function TaskCard({ task, onClick }: { task: Task; onClick: () => void }) {
  return <Card className="cursor-pointer p-3 hover:border-primary/30" onClick={onClick}><div className="flex items-start justify-between gap-2"><p className="text-sm font-medium text-text">{task.title}</p><span className={cn("h-2 w-2 rounded-full", task.priority === "urgent" ? "bg-error" : task.priority === "high" ? "bg-warning" : "bg-primary")} /></div><div className="mt-2 flex items-center justify-between"><span className="text-xs text-text-muted">{task.assigneeId ? task.assigneeId.slice(0, 8) : "Unassigned"}</span>{task.dueDate ? <span className="text-xs text-text-muted">{new Date(task.dueDate).toLocaleDateString()}</span> : null}</div></Card>;
}
...
function TaskDialog({ task, members, open, onOpenChange, onSave }: { task: Task | null; members: { userId: string }[]; open: boolean; onOpenChange: (open: boolean) => void; onSave: (body: Parameters<ReturnType<typeof useUpdateTask>["mutate"]>[0]["body"]) => void }) {
  ...
  if (!task) return null;
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent className="max-w-lg p-0"><DialogHeader><DialogTitle>Edit task</DialogTitle><DialogDescription>Update task details and workflow state.</DialogDescription></DialogHeader><div className="space-y-3 px-4 pb-4"><Input value={title} onChange={(event) => setTitle(event.target.value)} /><Input value={description} onChange={(event) => setDescription(event.target.value)} placeholder="Description" /><div className="grid grid-cols-2 gap-2"><select className={selectClass} value={status} onChange={(event) => setStatus(event.target.value)}>{statuses.map((item) => <option key={item} value={item}>{labels[item]}</option>)}</select><select className={selectClass} value={priority} onChange={(event) => setPriority(event.target.value)}>{priorities.map((item) => <option key={item}>{item}</option>)}</select><select className={selectClass} value={assigneeId} onChange={(event) => setAssigneeId(event.target.value)}><option value="">Unassigned</option>{members.map((member) => <option key={member.userId} value={member.userId}>{member.userId}</option>)}</select><Input type="date" value={dueDate} onChange={(event) => setDueDate(event.target.value)} /></div><div className="flex justify-between"><Button variant="ghost" className="text-error" onClick={() => deleteTask.mutate({ taskId: task.id, projectId: task.projectId }, { onSuccess: () => onOpenChange(false) })}><Trash2 className="mr-1 h-4 w-4" />Delete</Button><div className="flex gap-2"><Button variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button><Button onClick={() => onSave({ title: title.trim(), description: description.trim() || null, status, priority, assigneeId: assigneeId || null, dueDate: dueDate || null })}>Save</Button></div></div></div></DialogContent></Dialog>;
}
