...
function FilePreview({ file }: { file: FileRecord }) {
  ...
  return (
    <div className="flex min-h-[16rem] flex-col items-center justify-center gap-4 p-4 pt-0">
      ...
      <Button disabled={!getFileUrl(file)} onClick={() => { const url = getFileUrl(file); if (url) window.open(url, "_blank"); }}>
                  ...
                  <tr key={f.id} className="cursor-pointer border-b last:border-0 hover:bg-surface-elevated/50" onClick={() => setPreviewFile(f)}>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        <Button variant="ghost" size="icon" disabled={!f.downloadUrl && !f.url} onClick={(e) => { e.stopPropagation(); const url = f.downloadUrl ?? f.url; if (url) window.open(url, "_blank"); }} aria-label={`Download ${f.originalName}`}>
            ...
            {filtered.map((f) => (
              ...
              >
                ...
                <div className="mt-auto flex gap-1">
                  <Button variant="ghost" size="sm" disabled={!f.downloadUrl && !f.url} onClick={(e) => { e.stopPropagation(); const url = f.downloadUrl ?? f.url; if (url) window.open(url, "_blank"); }}><Download className="mr-1 h-3.5 w-3.5" />Download</Button>
