...
- Recruiter/interviewer home dashboard: `screens/home-recruiter.tsx` (both desktop apps) renders from `screens/home.tsx` when `useIsRecruitingRole()` matches (role name `recruiter`/`hiring_manager`/`interviewer`, or any `interview.*` permission on a non-candidate/guest/external member — checked after the admin/hr shell variants). It shows KPI cards + a 5-column candidate pipeline (backend stages grouped: applied / screening+shortlisted / interview+evaluation / offer / hired), today's interview schedule, recent activity (candidates + hiring decisions, gated on `interview.decision.view`), pending evaluations, and a real source-breakdown donut from `candidate.source`. Its tab strip deep-links into the Interview screen via the `interviewTab` param on `setActiveView` (`stores/ui.ts`); `CreateJobDialog`/`CreateCandidateDialog` accept an optional `trigger` element for custom buttons.
